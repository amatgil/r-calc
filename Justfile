run:
        RUSTFLAGS="-Zlocation-detail=none -Zfmt-debug" sudo cargo run --release --config "target.'cfg(target_arch = \"avr\")'.runner = 'ravedude -cb 57600 mega2560'"  --target="./avr-atmega2560.json" -Zbuild-std="core,panic_abort"  -Z build-std-features="optimize_for_size,panic_immediate_abort"
build:
